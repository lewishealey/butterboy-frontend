...
const PickDateTime = ({ onSchedule, settings }) => {
  ...
  const disabledClasses =
    "border border-gray-400 rounded bg-gray-100 text-gray-400 cursor-not-allowed px-4 py-2 w-full";
  const buttonClasses =
  ...
  return (
    <div className="flex w-full flex-col space-y-8">
      {!date && (
        <h2 className="font-display text-4xl text-center text-vibrant uppercase">
          Select date
        </h2>
      )}
      {!date && (
        <ul className="flex flex-col space-y-2 font-body text-xl w-full">
          {[...Array(settings.pickupRange)].map((elementInArray, index) => {
            let yourDate = today.addDays(index + settings.noticePickUp / 24); // noticePickUp / 24 = days
            ...
            if (index > 0) {
              return (
                ...
                >
                  {yourDate.toLocaleDateString(timezone, options)}
                </button>
              );
            }
          })}
      ...
      {date && time && (
        <div className="space-y-4">
          ...
          <button
            className="font-display uppercase text-vibrant bg-mauve py-4 text-3xl px-8 hover:bg-vibrant hover:text-mauve"
            onClick={scheduleDateTime}
          >
            Schedule
          </button>
        </div>
